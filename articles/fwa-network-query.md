...
`fresh` wraps fwapg and bcfishpass into composable R functions for
upstream/downstream network queries, habitat model filtering, and
waterbody traversal. All functions accept `table` and `cols` parameters
so the same interface works across FWA base tables and bcfishpass model
views.
