...
# --- Scenarios: 0% (baseline), 0.25%, 0.5%, 0.75% spawn gradient min ---
...
conn |>
  frs_classify("working.byman_habitat", label = "co_spawn_025",
    ranges = list(gradient = c(0.0025, spawn_max), channel_width = c(cw_min, 9999)),
    where = "accessible IS TRUE") |>
  ...
  frs_classify("working.byman_habitat", label = "co_spawn_075",
    ranges = list(gradient = c(0.0075, spawn_max), channel_width = c(cw_min, 9999)),
    where = "accessible IS TRUE")
...
and minimum spawning gradients. bcfishpass uses a minimum of 0% —
salmonids don’t spawn in flat water, so we default to 0.5%. Load with
...
(blue) carry the main flow.](figure/plot-network-1.png)
...
falls.](figure/plot-access-1.png)
...
reaches.](figure/plot-habitat-1.png)
...
reaches.](figure/plot-scenarios-1.png)
