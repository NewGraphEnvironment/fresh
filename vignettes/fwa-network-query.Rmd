---
title: "FWA Network Queries"
output: bookdown::html_vignette2
vignette: >
  %\VignetteIndexEntry{FWA Network Queries}
...
functions for upstream/downstream network queries, habitat model filtering,
and waterbody traversal. All functions accept `table` and `cols` parameters
so the same interface works across FWA base tables and bcfishpass model views.
...
Neexdzii Kwa / Wedzin Kwa confluence to pull the full stream network, then
zooming in to order 4+ coho rearing/spawning habitat from bcfishpass.
...
8876 stream segments in the full network narrow to
1027 coho habitat segments at order 4+ (orders
4, 5, 6).
363 lakes and 1293 wetlands upstream.
...
![Upstream network from the Neexdzii Kwa / Wedzin Kwa confluence. Light blue: all FWA streams. Coloured overlay: coho rearing and spawning habitat (order 4+) from bcfishpass, styled with the gq registry.](figure/plot-network-1.png)

## Summary

| Function | Purpose |
|----------|---------|
| `frs_network_upstream()` / `_downstream()` | Traverse the stream network |
| `frs_network_prune()` | Filter by order, gradient, or custom SQL |
| `frs_waterbody_network()` | Lakes/wetlands via waterbody_key bridge |
| `frs_stream_fetch()` | Fetch by watershed group, bbox, or blue line key |
| `frs_fish_obs()` / `frs_fish_habitat()` | Fish observations and habitat model |
