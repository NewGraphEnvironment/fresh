---
title: "Classifying Habitat Based on Accessibility and Intrinsic Habitat Potential"
output: bookdown::html_document2
params:
  update_gis: FALSE
vignette: >
  %\VignetteIndexEntry{Classifying Habitat Based on Accessibility and Intrinsic Habitat Potential}
...
don't spawn in flat water, so we default to 0.5%. Load with
...
database. Both write to the same breaks table.
...
![Accessible (blue) and inaccessible (grey) reaches. Red triangles mark access barriers. Black squares are all known falls.](figure/plot-access-1.png)
...
![Coho habitat classification within accessible reaches.](figure/plot-habitat-1.png)

### Lake rearing

bcfishpass scores lake-connected segments as `rearing = 0`
([bcfishpass#408](https://github.com/smnorris/bcfishpass/issues/408)).
With fresh we experiment to understand what changes when we classify them
independently using channel width thresholds and waterbody key filtering.
...
minimum gradient excludes low-gradient reaches where salmonids are unlikely
to spawn.
...
![Spawning habitat under four minimum gradient thresholds. Blue segments are classified as spawning within accessible reaches.](figure/plot-scenarios-1.png)
...
lengths. Here we aggregate upstream of road/stream crossings — structures
(culverts or bridges) identified by `aggregated_crossings_id` from
bcfishpass.
...
[**flooded**](https://newgraphenvironment.github.io/flooded/) (floodplain
delineation using upstream area and mean annual precipitation from
[`frs_col_join()`](https://newgraphenvironment.github.io/fresh/reference/frs_col_join.html)) and
[**drift**](https://newgraphenvironment.github.io/drift/) (land cover
change within floodplain extents).
